-- Atlas: Super Admin direct user creation and account deletion

CREATE OR REPLACE FUNCTION public.create_user_account(
  p_email TEXT,
  p_full_name TEXT,
  p_role TEXT DEFAULT 'user',
  p_password TEXT DEFAULT 'Atlas123!'
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_encrypted_pw := extensions.crypt(
    COALESCE(NULLIF(TRIM(p_password), ''), 'Atlas123!'),
...
BEGIN
  ...
  PERFORM set_config('atlas.allow_ledger_deletion', 'true', true);
  ...
  WHERE actor_id = p_profile_id OR target_id = p_profile_id;

  UPDATE public.flags SET resolved_by = NULL WHERE resolved_by = p_profile_id;
