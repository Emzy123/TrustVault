-- Atlas: 11-digit unique wallet account numbers for new registrations / creations
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    role = CASE
      WHEN EXCLUDED.email IN ('admin@atlas.demo', 'superadmin@atlas.demo') THEN v_role
...
BEGIN
  ...
  v_password := COALESCE(NULLIF(TRIM(p_password), ''), 'Atlas123!');
