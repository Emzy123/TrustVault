-- Atlas Master Database Setup & Auth Seed Script
--
...
BEGIN
  IF current_setting('atlas.allow_ledger_deletion', TRUE) = 'true' THEN
...
BEGIN
  IF OLD.balance IS DISTINCT FROM NEW.balance THEN
    IF current_setting('atlas.allow_ledger_posting', TRUE) IS DISTINCT FROM 'true' THEN
...
DECLARE
  ...
  v_tx_id UUID;
BEGIN
  v_alice_id := public.seed_auth_user('alice@atlas.demo', 'Password123!', 'Alice Vance');
  v_bob_id := public.seed_auth_user('bob@atlas.demo', 'Password123!', 'Bob Builder');
  v_charlie_id := public.seed_auth_user('charlie@atlas.demo', 'Password123!', 'Charlie Brown');
  v_admin_id := public.seed_auth_user('admin@atlas.demo', 'Password123!', 'Admin Officer');
  v_superadmin_id := public.seed_auth_user('superadmin@atlas.demo', 'Password123!', 'Super Admin Officer');

  -- Profiles
  INSERT INTO public.profiles (id, full_name, email, role, account_status, kyc_status)
  VALUES (v_alice_id, 'Alice Vance', 'alice@atlas.demo', 'user', 'active', 'approved')
  ON CONFLICT (id) DO UPDATE SET role = 'user', account_status = 'active', kyc_status = 'approved';

  INSERT INTO public.profiles (id, full_name, email, role, account_status, kyc_status)
  VALUES (v_bob_id, 'Bob Builder', 'bob@atlas.demo', 'user', 'verified', 'approved')
  ON CONFLICT (id) DO UPDATE SET role = 'user', account_status = 'verified', kyc_status = 'approved';

  INSERT INTO public.profiles (id, full_name, email, role, account_status, kyc_status)
  VALUES (v_charlie_id, 'Charlie Brown', 'charlie@atlas.demo', 'user', 'unverified', 'not_submitted')
  ON CONFLICT (id) DO UPDATE SET role = 'user', account_status = 'unverified', kyc_status = 'not_submitted';

  INSERT INTO public.profiles (id, full_name, email, role, account_status, kyc_status)
  VALUES (v_admin_id, 'Admin Officer', 'admin@atlas.demo', 'admin', 'verified', 'approved')
  ON CONFLICT (id) DO UPDATE SET role = 'admin', account_status = 'verified', kyc_status = 'approved';

  INSERT INTO public.profiles (id, full_name, email, role, account_status, kyc_status)
  VALUES (v_superadmin_id, 'Super Admin Officer', 'superadmin@atlas.demo', 'super_admin', 'verified', 'approved')
  ...
  RAISE NOTICE 'Atlas database setup and pre-confirmed auth accounts created successfully.';
END $$;
