...
if [[ -z "${SUPABASE_URL:-}" || -z "${SUPABASE_ANON_KEY:-}" ]]; then
  echo "ERROR: Set SUPABASE_URL and SUPABASE_ANON_KEY in the Vercel project Environment Variables." >&2
  echo "Atlas uses a separate Supabase project from TrustVault." >&2
  exit 1
fi
